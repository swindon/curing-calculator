...
function app() {
    return {
        ...
        update_calc: function() {
            var ppm_value = document.querySelector('[data-name="ppm"]').value;
            var wt_factor = 1;
            var vol_factor = 1.0;
            var total_vol = vol_factor * document.querySelector('[data-name="brine_amt"]').value + wt_factor * document.querySelector('[data-name="meat_amt"]').value;
            var gms_no2 = total_vol * 1000 * ppm_value / 1000000 / 0.0625;
            this.calc_salt = (gms_no2).toFixed(1) + " grams";
        },
        update_dims: function() {
            var t_factor = 1 / 2.54;
            var tdim = t_factor * document.querySelector('[data-name="thick_ness"]').value;
